...
@onready var inventory_ui: Control = $InventoryUIRedSaber# Generic name, adjust in the scene as needed

@export var speed : int = 500
@export var max_horizontal_speed : int = 300
@export var slow_down_speed : int = 1700
@export var player_id: int = 6  # Assign a unique ID (0-6) in the inspector for each player
...
func _input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		var main_scene = get_tree().root.get_node("Main Scene")
		if main_scene:
			main_scene.select_character(self)
